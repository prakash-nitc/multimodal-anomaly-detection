...
That account is a conjecture consistent with two datasets, not a demonstrated
relationship, and we mark it as such. It is testable: the prediction is that
partitioning ShanghaiTech by camera view and running the sweep within a single
view should reproduce Avenue's flat result, while the pooled version reproduces
the effect. That experiment is not yet run.
...
\emph{The adaptation mechanism is bounded, and a second domain establishes
where.} Applied unchanged to CUHK Avenue, the framework detects equally well
($0.706$ against $0.707$) but its context effect nearly vanishes: the gap falls
from $+0.105$ to $+0.020$ and an accurate descriptor scores below no descriptor
at all. Avenue is a single camera view where ShanghaiTech is thirteen, which
suggests that verbalized context earns its place in proportion to the domain
variation it must resolve. On the evidence of two datasets that is a conjecture
rather than a law, but it is a falsifiable one, and it converts an unqualified
claim into a scoped one.
...
\begin{itemize}
  ...
  \item The adaptation mechanism is demonstrated on one of the two video domains
        evaluated. On CUHK Avenue the effect is five times smaller and an
        accurate descriptor does not improve on none, so the claim is scoped to
        settings with scene variation for the descriptor to resolve. The
        explanation offered for that boundary rests on two datasets and is not
        established.
...
\begin{itemize}
  \item The within-view control described in Section~\ref{sec:res-avenue}:
        partitioning ShanghaiTech by camera and running the sweep inside a
        single view. If the effect disappears there, scene diversity is
        established as the operative variable rather than conjectured.
