...
14 Aug 2026 & Positioned against concurrent work. \citet{wilkinghoff2026context}
         argue independently that normality is context-dependent and that
         abnormality should be assessed conditionally. Observation~2 and
         Contribution~C1 revised to cite it and to state precisely what this work
         adds: the domain-adaptation vocabulary that locates the mismatch,
         language rather than additional sensing as the context channel, and a
         protocol that tests whether supplied context does any work. \\
...
web scale, such as CLIP \citep{radford2021clip}, permit recognition of concepts
specified purely in natural language and are, as a consequence of their training
breadth, substantially robust to the appearance variation that covariate-shift
methods were built to correct. If a foundation model already absorbs much of the
...
The context-dependence of normality has been argued independently and
concurrently by \citet{wilkinghoff2026context}, who observe that ``a specific
observation may be normal under one operating condition, yet anomalous under
another'', that modelling normality against a single unconditional reference
therefore introduces ``structural ambiguity'', and that abnormality should be
defined conditionally rather than relative to a global reference. We take that
convergence as support for the premise rather than as competition for it, and
our position differs from theirs in three respects. First, we state the problem
in the formal vocabulary of the domain-adaptation literature, which allows the
mismatch of Observation~1 to be identified precisely: the shift anomaly
detection exhibits is the one that literature scopes out. Second, their context
is carried by additional sensing modalities, whereas ours is carried by
natural language supplied by an operator, which requires no additional
instrumentation. Third, and most consequentially, their contribution is a
position: they argue for context-conditional assessment and enumerate open
challenges. The present work supplies a mechanism and, more importantly, a
protocol that can determine whether the supplied context does any work at all.
...
\begin{enumerate}[C1.]
  \item \textbf{Characterisation.} We formalise concept shift, rather than
        ...
        literature, which locates the mismatch precisely---the shift anomaly
        detection exhibits is the one that literature excludes by explicit
        scoping decision---and explains why $p(x)$-alignment methods are
        structurally unsuited to the task.
...
\begin{enumerate}[Stage 1.]
  ...
  \item \textbf{Frame scoring.} Each frame is embedded by the frozen image
        ...
        the frame matches the abnormal prototype rather than the normal one,
        computed over the two cosine similarities and scaled by the temperature
        $\lambda$ learned during the backbone's own pre-training
...
\begin{description}
  ...
  Transformer \citep{dosovitskiy2021vit}, itself built on the transformer
  architecture of \citet{vaswani2017attention}, which partitions the frame into
  $14 \times 14$ patches and produces a single unit-norm embedding in the joint
  image--text space that CLIP learns by contrastive pre-training. Scoring
...
such as that of \citet{zanella2024lavad}, which chains a captioner, a large
language model and a cross-modal refiner.
...
12.1, using CLIP ViT-L/14 pretrained on LAION-2B. Each run records a manifest
capturing the commit identifier of the code, the hardware, the library versions,
the full configuration, and the per-dataset sequence, frame and positive-label
counts, so that every reported figure is traceable to the state that produced
it. Image embeddings are cached separately from scores: the visual encoder is
...
Because the baseline consumes identical features and is evaluated on the
identical test split, the only difference between the two rows of
Table~\ref{tbl:mvtec} is whether training on normal data took place; the
comparison therefore isolates the value of that training.
...
Each ensemble is mean-pooled into a single embedding, so text appended to both
ensembles enters both prototypes as a common component, drawing them together
and contracting the margin that carries the decision. The more faithfully the
descriptor describes the imagery, the more strongly it aligns with every frame,
and the more of the normal/abnormal contrast it absorbs---which is precisely why
an \emph{accurate} description was the most damaging one, and why a description
of an unrelated domain, aligning with nothing in the scene, left the original
...
the semantic signal when combined with it. Separately, quadrant-level scoring
with maximum aggregation---intended to recover anomalies too small to survive
the $224\times224$ resize---did not improve on whole-frame scoring in any
configuration; mean aggregation over quadrants was uniformly superior, which is
consistent with mild ensembling rather than with recovery of spatial detail.
...
dilution mechanism of Section~\ref{sec:res-sweep}: the domain-specific normal
and abnormal prompts share heavy scene vocabulary (``walkway'', ``pedestrians''),
which contracts the margin between prototypes in exactly the manner the fusion
experiment isolated, while their added specificity becomes useful once a
separate signal establishes that the named object is in anomalous motion.
...
respectively on ShanghaiTech, and $0.706$ in combination. That the kinematic
signal---which consults no language at all---is competitive with the semantic
one is a caution against reading frame-level vision-language scores on this
benchmark as evidence of semantic understanding, and indicates that a
substantial part of what such scores capture on kinematically defined anomalies
is temporal change.
...
context-dependence comes from varying the descriptor within one domain, which is
a necessary but not a sufficient condition for adaptation across domains.
